...
语句 `items("//*")` 会检索出所有的组件对象，而 语句 `items("//Button")` 则只会检索出所有的按钮对象。注意，由 `items` 函数检索出的对象都是值对象。
...
该示例中，由于检索函数均带有上下文，所以检索范围就被限制于上下文之内。语句 `items("button", sys.index)` 检索出按钮对象 bar，而语句 `sys("button", sys.sub)` 则检索出按钮对象 foo。需要注意的是，检索语句检索的范围并不包含上下文本身。
