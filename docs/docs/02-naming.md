...
在程序运行时，视图项中的任何节点都会实例化为对象。虽然，对于未命名节点的相应组件对象无法直接显示使用，但我们完全有办法获得并使用它，在后面章节中将会看到这点是如何做到的。
...
有时候，会存在同名的集体对象和个体对象。这种情况下，个体对象具有优先权。也就是说，个体对象的样式会覆盖集体对象的样式。
